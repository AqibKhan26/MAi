class AI {
      ...
      for (int j = 0; j< tilesY; j++) {
        if (!tiles[i][j].hidden) {
          if (tiles[i][j].bombsNear == tiles[i][j].flaggedNear) {
            ...
            println("well,fuck me dead son! i cant handle this!");//error message
          }
        }
      }
    }
          ...
          if (tiles[i][j].bombsNear != 1 &&  tiles[i][j].bombsNear != 0 && tiles[i][j].bombsNear - tiles[i][j].flaggedNear > 1 ) {
            for (int k = 0; k < tiles[i][j].hiddensNear.size(); k++) {//for each hiddes tile near the tile being checked
              ...
              if (tiles[i][j].hiddensNear.get(k).linked) {//if that tile is linked
                ...
                if (numberLinked >1) {
                  //now we know that there are atleast 2 linked tiles adjacent to this tile
                  //we can reduce the number of spaces left to put bombs by numberLinked -1
  ...
  void clickTarget() {
    ...
    if (!flagTarget) {
      if (temp.flagged) {//if clicking an already flagged target.
        print("No can do");
      } else {
  ...
  void setProbabilities() {
    ...
    //unknown tiles are tiles we have no information about
    int unknownCount = 0;
    for (int i = 0; i< tilesX; i++) {//for each tile
      for (int j = 0; j< tilesY; j++) {
        if (tiles[i][j].hidden) {
    ...
    //now we have an arrayList containing all the possible lists of bomb configurations
  ...
  ArrayList<ArrayList<Tile>> getSolutions(ArrayList<Tile> section) {
    ...
    while (true) {
      ...
      if (solution(checking, section)) {
        ...
        if (!violated(checking, checkingInt, hiddensInSection, section)) {//if it has voilated the ruiles of minesweeper then ignore it
